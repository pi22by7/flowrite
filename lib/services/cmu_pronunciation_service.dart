...
class CMUPronunciationService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      debugPrint('Error loading CMU dictionary from assets: $e');
  ...
  Future<void> _loadFromAssets() async {
    try {
      debugPrint('Loading CMU pronunciation dictionary from assets...');
      final String content = await rootBundle.loadString(_assetPath);
      final lines = LineSplitter.split(content);
      _parseDictionary(lines);

      debugPrint('CMU dictionary loaded: ${_pronunciationDict.length} entries');
    } catch (e) {
      debugPrint('Error loading CMU dictionary from assets: $e');
      rethrow;
    }
  ...
  void _parseDictionary(Iterable<String> lines) {
    ...
    for (final line in lines) {
      ...
      if (parts.length >= 2) {
        ...
        final phonemes =
            parts.sublist(1); // All parts after the word are phonemes
  ...
  List<String> findRhymes(String targetWord, List<String> words) {
    ...
    return words.where((word) => doWordsRhyme(targetWord, word)).toList();
  }
